...
void detectMono(
  ...
  if (configuration.edge_detection.apply) {
    ...
    cv::Canny(
      ...
      configuration.edge_detection.min_threshold, configuration.edge_detection.max_threshold);
    processed = canny_filtered;
  }
  ...
  circles.erase(
    std::remove_if(
      ...
      [configuration](cv::Vec3f p){return !configuration.roi.contains(cv::Point(p[0], p[1]));}),
    circles.end());
  ...
  if (circles.size() > 4) {
    ...
    for (const auto & circle : circles ) {
      distances_from_median.push_back(sqrt(
        (circle[0] - median_x) * (circle[0] - median_x) +
        (circle[1] - median_y) * (circle[1] - median_y)));
    }
  ...
  if (circles.size() != 4) {
    throw DetectionException("Number of circles found: " + std::to_string(circles.size()) +
                             ", but should be exactly 4.");
  }
  ...
  std::sort(centers.begin(), centers.end(), [origin](cv::Point2f a, cv::Point2f b) {
    return std::atan((a.y - origin.y) / (a.x - origin.x)) >
           std::atan((b.y - origin.y) / (b.x - origin.x));
  });
