	...
	nh_(nh)
{
	...
	nh_.param<float>("minimum_RCS", min_RCS_, -std::numeric_limits<float>::max());
	nh_.param<float>("maximum_RCS", max_RCS_, std::numeric_limits<float>::max());
	nh_.param<float>("min_range_object", min_range_object_, -std::numeric_limits<float>::max());
...
void RadarDetectorNode::callback(radar_msgs::RadarDetectionArray const & in) {
	...
	pcl::PointXYZ point = keypointDetection(in, min_RCS_, max_RCS_,min_range_object_, max_range_object_); // Z-Value is actually the rcs value
